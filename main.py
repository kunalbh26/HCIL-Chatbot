...
# IMPORTANT: Place your 'dataset.xlsx' file in the same directory as this script,
# or provide the correct path (e.g., 'data/dataset.xlsx').
...
/* Main Chat Area Background & Padding for Pinned Input Bar */
.main {{
    background: #1F1F1F;
    border-radius: 0px;
    padding: 1rem 1rem 15rem 1rem !important; /* Added bottom padding to prevent overlap with pinned bar */
    max-width: 640px;
    margin: 2.0rem auto;
}}
...
/* ### FIX 1: CSS for the new 'Start Chat' button container and button ### */
.start-chat-container {{
    display: flex;
    justify-content: center;
    align-items: center;
    height: 50vh; /* Use viewport height to help center vertically */
...
.start-chat-container .stButton button {{
    ...
    color: #fff;
    border-radius: 20px;
    padding: 1.2rem 2.5rem; /* Increased padding to make button bigger */
    cursor: pointer;
...
/* ### FIX 3: PINNED INPUT BAR CONTAINER ### */
...
/* Adjust the main content area for the pinned bar when sidebar is open/closed */
/* This targets the main block container Streamlit uses */
.block-container {{
    transition: all 0.3s ease-in-out;
}}

/* When sidebar is open, the main content is pushed. We need to adjust our pinned bar. */
/* Streamlit adds a 'transform' style to the main container when the sidebar opens. */
/* We use a more general approach to find the main content area and adjust our pinned bar's position. */
.main > .block-container {{
     padding-bottom: 15rem; /* Ensure space for pinned bar */
}}

.pinned-input-bar-inner {{
    max-width: 640px; /* Match the width of the main chat content */
    margin: 0 auto;
}}

.pinned-input-bar-inner .stTextInput > div > div > input {{
    background: #323232;
    border: 2px solid #ff0000;
    color: #fff;
    width: 100%;
...
.pinned-input-bar-inner .stButton button {{
    background: linear-gradient(90deg, #e53935 0%, #b71c1c 100%);
    color: #fff;
    border: none;
    border-radius: 50%;
...
.pinned-input-bar-inner .stButton button:hover {{
    background: #fff;
    color: #e53935;
...
.feedback-container .stButton button:hover {{
    transform: scale(1.1);
    border-color: #e53935;
}}
...
def is_gibberish(text):
    ...
    words = text.split()
    if len(words) > 0 and sum(1 for w in words if not w.isalpha()) / len(words) > 0.5: return True
    return False
...
def render_chat(messages):
    for message in messages:
        if message["role"] == "user":
            st.markdown(f'<div class="user-row"><div class="chat-bubble user-bubble">{message["content"]}</div><div class="avatar" style="margin-left:8px;">🧑‍💻</div></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="bot-row"><div class="avatar" style="margin-right:8px;">🤖</div><div class="chat-bubble bot-bubble">{message["content"]}</div></div>', unsafe_allow_html=True)
...
# This is the main container for the chat history
...
with chat_container:
    ...
    if not st.session_state.chat_started:
        # ### FIX 1: USE A CONTAINER FOR CENTERING THE 'START CHAT' BUTTON
        with st.container():
        ...
        # This block runs only after 'Start Chat' is clicked
        if st.session_state.get('knowledge_base_loaded', False):
            ...
            # ### FIX 2: SHOW QUICK REPLIES ONLY AT THE START OF THE CHAT
            # Only show if there is just one message (the bot's initial greeting)
            if len(st.session_state.messages) == 1:
                ...
                for i, reply in enumerate(st.session_state.quick_replies):
                    if cols[i].button(reply, key=f"quick_reply_{reply}", use_container_width=True):
                        st.session_state.messages.append({"role": "user", "content": reply})
                        st.session_state.show_typing = True
                        st.rerun()
                st.markdown('</div>', unsafe_allow_html=True)

# ### FIX 3: PINNED INPUT BAR AT THE BOTTOM ###
if st.session_state.chat_started and not st.session_state.chat_ended:
    ...
    st.markdown('<div class="pinned-input-bar-inner">', unsafe_allow_html=True)

    # Feedback Reactions (now inside the pinned bar)
    if st.session_state.feedback_request:
        st.markdown('<div class="feedback-container">', unsafe_allow_html=True)
        col1, col2, col3, col4 = st.columns(4)
        if col1.button("👍", key="feedback_like", use_container_width=True):
            st.session_state.messages.append({"role": "bot", "content": "Great! Let me know if there is something else I can help you with."})
            st.session_state.feedback_request = False
            st.rerun()
    ...
    # Pinned Input Form
    with st.form("chat_input_form_pinned", clear_on_submit=True):
        col1, col2 = st.columns([6, 1])
        with col1:
            user_input = st.text_input("user_input", placeholder="Type here...", key="input_bar_pinned", label_visibility="collapsed")
        with col2:
        ...
        if send_clicked and user_input.strip():
            ...
            if user_input_clean in ["bye", "end", "quit"]:
                ...
                st.session_state.show_typing = True
                st.session_state.feedback_request = False # Hide feedback buttons while processing
                st.rerun()
                
    st.markdown('</div>', unsafe_allow_html=True) # close pinned-input-bar-inner
    st.markdown('</div>', unsafe_allow_html=True) # close pinned-input-container


# Bot response logic (remains at the end to be processed after input)
if st.session_state.get("show_typing", False):
